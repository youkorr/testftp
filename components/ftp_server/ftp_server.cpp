...
void FTPServer::start_file_download(int client_socket, const std::string& path) {
  ...
  char* write_buffer = buffer2;

  // Lire d'abord dans read_buffer
  int bytes_read = read(file_fd, read_buffer, BUFFER_SIZE);

  while (bytes_read > 0) {
    // Commencer la prochaine lecture dans l'autre buffer
    char* next_buffer = (read_buffer == buffer1) ? buffer2 : buffer1;
    int next_bytes = read(file_fd, next_buffer, BUFFER_SIZE);

    // Envoyer le buffer actuel pendant que la prochaine lecture se prepare
    send(data_socket, read_buffer, bytes_read, 0);
